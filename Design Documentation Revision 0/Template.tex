...
\begin{enumerate}[]
	...
	\item \textbf{Requirement Reference:} 8.1(c), 11.5(a)
...
\begin{enumerate}[]
	...
	setEventList(eventList):
		\begin{itemize}
			\item Input and store the event list
		\end{itemize}
	\item \textbf{Process Steps:} This module is initialized by the simulation core and the state data structure is set to null.  After initialization the the simulation core will populate the state through function calls.  The module will now function as an interface to the state data structure.
	\item \textbf{Data:} State data structure
	\item \textbf{Error Handling:} Catch any null values returned by the state data structure 
	\item \textbf{Requirement Reference:} 11.5(a)
	\item \textbf{Critical Revision 0 Component:} True
\end{enumerate}
	...
	\item \textbf{Interface:} \newline
	 ...
	 getProxmityMatchValue(Job Origin):
	 	\begin{itemize}
	 		...
	 		\item Returns a value based on how close an available porter is to a job's origin
	 	\end{itemize}
	 getWeightedJobPriority(Job Origin, Job Destination):
	 	\begin{itemize}
	 		\item Input the origin and destination of a pending job
	 		\item Returns a value based on the priority of the pending job
	 	\end{itemize}
	 getAppointmentFactor(Job):
	 	\begin{itemize}
	 		\item Input a pending job
	 		\item Returns the value for a job depending on if it was pre-scheduled or on-demand.
	 	\end{itemize}
